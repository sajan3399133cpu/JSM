...
scene_files = []
print(f"🎬 Total Scenes to Render: {len(kws)} - RAM SAFE MODE ON")
...
for idx, ch in enumerate(kws):
    ...
    if not ok:
        print(f"⚠️ Voiceover Failed Scene {idx+1}")
        continue
    au = AudioFileClip(ap)
    ...
    gc.collect()
    print(f"✅ Scene {idx+1} Done & Saved - RAM Cleared")

if scene_files:
    print("\n🔗 Joining all scenes (FFMPEG - Zero RAM)...")
    ...
    print(f"\n🎉 SUCCESS! Video Ready: {out_path} - {len(scene_files)} scenes, NO RAM CRASH")
    try:
        final_clip = VideoFileClip(out_path)
        final_mins = round(final_clip.duration / 60, 2)
        final_clip.close()
        email_for_cut = globals().get('user_email', 'areej3399133@gmail.com')
        cut_mints_auto(email_for_cut, final_mins)
    except Exception as e:
