...
def clean_analyze(script):
 ...
 # FIX 1: Ab 3 lafz wala jumla bhi chalega
...
def Kw(text,cat):
 ...
 if any(x in l for x in ["trump","iran","missile","war"]): return "political news war military"
 if any(x in l for x in ["imran khan","pakistan","protest","long march"]): return "pakistan politics protest rally"
 w=[x for x in re.findall(r'\w+',l) if len(x)>4][:3]
 return " ".join(w)+" professional cinematic 4k" if w else "nature cinematic 4k"
def get_category(text):
 l=text.lower()
 if any(x in l for x in ["trump","iran","war","military"]): return "politics"
 if any(x in l for x in ["imran khan","pakistan","protest"]): return "pakistan politics"
 return "general"
def Ai(p,path,W=960,H=540):
 ...
 Image.new('RGB',(W,H),color=(10,10,10)).save(path)
 return path
def St(k,d,W,H,cat):
 q=Kw(k,cat)
 for key in XK:
 ...
 return ColorClip((W,H),color=(10,10,10),duration=d)
def MakeSEO(s):
 l=s.lower()
 if any(x in l for x in ["trump","iran","war"]):t="World Politics & Breaking News"
 elif any(x in l for x in ["imran khan","pakistan"]):t="Pakistan Politics"
 else:t="Latest News Update"
 b=s[:70].strip().replace("\n"," ")
 title=f"{b} | {t} 2026"
 desc=f"{s[:500]}\n\nDisclaimer: This is AI generated news summary for informational purposes.\n"
 ht=f"#{t.replace(' ','')} #BreakingNews #LatestUpdate"
 tags=f"{t}, {b}, Breaking News 2026"
...
def Gen(email,code,script,lang,vtype,res,show_sub,cat_hidden):
 ...
 # FIX 2: Agar 1 bhi jumla na mila to error na de
 if not kws: return None,None,"","","","Script me jumle nahi mile.. ya! lagao"

 try:
  need=0.0;USED.clear()
  for idx,ch in enumerate(kws):
   print(f"Processing Part {idx+1}/{len(kws)}")
   ap=f"/tmp/{uuid.uuid4().hex[:5]}.mp3"
   run_tts(ch,ap,VOICES.get(lang,"en-US-GuyNeural"))
   if not os.path.exists(ap) or os.path.getsize(ap)<1000:continue
   ...
   for i in range(num_clips):
    if i>0 and i%3==0: time.sleep(1) # Trip rokne ke liye
    ...
    if clip_dur<0.8:clip_dur=per_clip
    # FIX 3: Agar clip me error aaye to skip kar do, puri video na roke
    try:
    ...
    except Exception as e: print(f"Clip Skip: {e}"); continue
  ...
  if not pvs:return None,None,"","","","Koi bhi clip nahi bani. Script check karo"
...
with gr.Blocks(title="JSM VIDEO GENERATOR V6.9") as demo:
 gr.HTML(f"""<div id="header"><h1>✦ JSM VIDEO GENERATOR V6.9 ✦</h1><div class="owner-info">OWNER: {ON} - {ONUM} | MANAGER: {MN} - {MNUM}</div></div>""")
 with gr.Tab("🎬 Video Generator"):
  ...
  script=gr.Textbox(lines=8,label="Your Script - News/Story/20 Min")
