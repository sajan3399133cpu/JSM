    ...
    try:
        ...
        if r.status_code == 200:
            for doc in r.json().get('response', {}).get('docs', []):
                id = doc['identifier']
                lk = f"https://archive.org/download/{id}/{id}.mp4"
                if lk not in USED:
                    USED.add(lk)
                    t = f"/tmp/{uuid.uuid4().hex[:4]}.mp4"
                    open(t, 'wb').write(requests.get(lk, timeout=10).content)
...
with gr.Blocks(title="JSM VIDEO GENERATOR V6.14") as demo:
    gr.HTML(f"""<div id="header"><h1>✦ JSM VIDEO GENERATOR V6.14 - 7 SOURCE ✦</h1><div class="owner-info">OWNER: {ON} - {ONUM} | MANAGER: {MN} - {MNUM}</div></div>""")
