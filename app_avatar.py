...
def get_avatar_clip(duration, W, H):
    try:
        ...
        path = globals().get('avatar_path','/content/avatar.png')
        if "No Avatar" in mode: return None
        if not os.path.exists(path):
            # try download if URL given
            url = globals().get('avatar_url','')
            if url and url.startswith("http"):
                r = requests.get(url, timeout=10)
                if r.status_code==200:
                    open(path,'wb').write(r.content)
        if not os.path.exists(path): return None
        # Avatar size - 28% of width
        avatar = ImageClip(path).set_duration(duration)
        # Make circle mask type positioning
        av_w = int(W*0.28)
        av_h = int(av_w * 1.3)
        avatar = avatar.resize((av_w, av_h)).set_pos((0.02, 0.05), relative=True)
        # Add subtle zoom
        avatar = avatar.resize(lambda t: 1 + 0.01*t)
        return avatar
    except Exception as e:
        print(f"Avatar error: {e}")
        return None

print(f"🎙️ Voice: {voice_lang} | Mode: {video_mode} | Avatar: {avatar_mode}")
